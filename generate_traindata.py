...
def split_clinlical_match(clinlical_match):
    ...
    for trial in keys:
        ...
        for example in clinlical_match[trial]['inc']:
            ...
            if total_inc < 21:
                if example not in new_clinlical_match:
                    new_clinlical_match[trial] = {}
                    new_clinlical_match[trial]['inc'] = list()
                    new_clinlical_match[trial]['exc'] = list()
                new_clinlical_match[trial]['inc'].append(example)
            
        for example in clinlical_match[trial]['exc']:
