        ...
        for idx, word in enumerate(context):
            ...
            if current_class and pred == 0:
                ...
                out_file.write(
                    ...
                    {'pointer_text': str(pred), 'rgb': attention_to_rgb(0.2), 'word': word,
                ...
                out_file.write(
                    '<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                    {'pointer_text': str(pred), 'rgb': attention_to_rgb(1), 'word': word,
                     'class': str(example_index) + "-" + word.replace("@", "")})
                out_file.write('</u>')
                ...
                out_file.write(
                    '<mark class="g%(class)s" cursor="pointer" title="%(pointer_text)s" style="background-color:%(rgb)s;">%(word)s </mark>' %
                    {'pointer_text': str(pred), 'rgb': attention_to_rgb(0.5), 'word': word,
                     'class': str(example_index) + "-" + word.replace("@", "")})
                out_file.write('</u>')
            else:
                out_file.write(
